  ...
  */
  track(event, properties) {
    return new Promise((resolve, reject) => {
      if (!this.initialized) return reject(new Error(uninitializedError('track')));
  
      if (this.webhook_events.contains(event)) {
        if (this.webhook_uri == null || this.webhook_uri == undefined) {
          return reject(new Error('Webhook attempting to be invoked for event ' + event + ' has no uri associated in property webhook_uri'));
        }
  
        // CASE: dispatch webhook
        fetch(this.webhook_uri, {
          method: 'POST',
          headers: {
        ...
        .then(response => {
          if (!response.ok) {
            return reject(new Error('Failed to flush events to webhook at ' + this.webhook_uri));
          }
          // Continue to the next step after webhook
          if (properties) {
        ...
        .catch(err => {
          reject(new Error('Failed to flush events to webhook at ' + this.webhook_uri + " for error: " + err));
        });
      } else {
        // If not in webhook_events, just track
        if (properties) {
          RNMixpanel.trackWithProperties(event, properties, this.apiToken).then(resolve).catch(reject);
        } else {
          RNMixpanel.track(event, this.apiToken).then(resolve).catch(reject);
        }
      }
    });
  }
